...
def preprocess(news_data, nobelow):
    ...
    for i in tqdm(range(len(news_df['summary']))):
        title_list.append(get_key_tokens(news_df.loc[i,'summary']))
        ...
        for i in range(3):
            ...
            if date_diff.days > 30:
                if frequency >= num_news_threshold:
                    ...
                    timeline_df = timeline_df.append({'ID': id_list, 'Keywords': topic_news["Keywords"].iloc[0], 'Date': date, 'Summary':title_list}, ignore_index=True)
                    prev_date = date
    print(timeline_df)
    return timeline_df
...
def topics_to_timeline(news_df, ldamodel, corpus, num_keywords, num_topics, perc_threshold):
    ...
    timeline_df = pd.DataFrame(columns = ['ID', 'Keywords', 'Date', 'Summary'])
    for i in range(1,num_topics+1):
