...
def topics_to_mongo(news_df, ldamodel, corpus, texts, num_keywords, num_topics):
    # Init output
    ...
    os.mkdir("한겨레_title_" + str(num_topics))
    for i in range(1,num_topics+1):
        ...
        globals()['df_{}'.format(i)].to_csv (".\\한겨레_title_" + str(num_topics) + "\\topic("+str(i)+")_news.csv", index = None)
...
if __name__ == '__main__':
    ...
    start = 20 # 이 범위는 뉴스 개수에 따라 다르게 하기
    limit = 91
    step = 10
    ...
    for tn in topic_priority:
        ...
        topics_to_mongo(news_df, ldamallet, corpus, title_list, 3, tn) # 마지막 인자 키워드 개수
