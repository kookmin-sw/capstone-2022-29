    ...
    if len(topic_news) != 0:
        ...
        for key, value in histogram.items():
            ...
            if value >= num_news_threshold:
                ...
                timeline_df = timeline_df.append({'ID': id_list, 'Keywords': topic_news["Keywords"].iloc[0], 'Date': key, 'Title':title_list}, ignore_index=True)
...
if __name__ == '__main__':
    ...
    topic_priority = [15, 25, 35]
    perc_list = [0.06, 0.05, 0.04]
    for i in range(len(topic_priority)):
        ...
        timeline = topics_to_timeline(news_df, ldamallet, corpus, 3, topic_priority[i], 0.05) # 마지막 인자 키워드 개수
