    ...
    if len(topic_news) != 0:
        ...
        for key, value in histogram.items():
            ...
            if value >= num_news_threshold:
                ...
                timeline_df.append({'ID': id_list, 'Keywords': topic_news["Keywords"].iloc[0], 'Date': key, 'Title':title_list}, ignore_index=True)
...
def topics_to_timeline(news_df, ldamodel, corpus, num_keywords, num_topics, perc_threshold):
    ...
    for i in range(1,num_topics+1):
        ...
        timelining(perc_threshold, 2, globals()['df_{}'.format(i)], timeline_df)
...
if __name__ == '__main__':
    ...
    start = 3 # 이 범위는 뉴스 개수에 따라 다르게 하기
    limit = 29
    step = 5
    ...
    for i in range(len(topic_priority)):
        ...
        timeline = topics_to_timeline(news_df, ldamallet, corpus, 3, topic_priority[i], 0.06) # 마지막 인자 키워드 개수
        print(timeline)
        topics = []
